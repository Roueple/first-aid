...
### Components
- **BernardPage.tsx**: Main chat interface with sidebar
- **BernardService.ts**: AI chat service with session integration
- **BernardSessionService.ts**: Session management (create, update, list)
- **BernardChatService.ts**: Chat message storage and retrieval
...
- `bernard_sessions`: Session metadata (title, messageCount, timestamps)
- `bernard_chats`: Individual chat messages (user and assistant)

### Indexes
All required Firestore indexes are configured in `firestore.indexes.json`:
- `bernard_sessions`: userId + isActive + lastActivityAt
- `bernard_sessions`: userId + createdAt
- `bernard_chats`: sessionId + timestamp (both directions)
- `bernard_chats`: userId + timestamp
...
All styles are in `src/renderer/styles/bernard.css`:
