...
/**
 * BernardGreetingService - Generates personalized greetings for Bernard
 * 
...
export class BernardGreetingService {
  /**
  ...
  private async getUsagePattern(userId: string): Promise<GreetingContext['usagePattern']> {
    const sessions = await BernardSessionService.getUserSessions(userId, 30);
...
export default new BernardGreetingService();
